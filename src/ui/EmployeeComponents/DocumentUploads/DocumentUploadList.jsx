...
import useDocumentUploads from "../../../hooks/documentHooks/useDocumentUploads";
...
const DocumentHistory = () => {
    ...
    const { content, totalPages, loading, error } = useDocumentUploads(page);
    ...
    return (
        <div className="max-w-6.5xl mx-auto py-6 px-4">
            ...
            <div className="space-y-2">
                {content?.map((item) => (
                    <DocumentUploadRow key={item.documentId} document={item} />
                ))}
            </div>
            ...
            {content?.length === 0 && (
                <div className="text-center py-12 bg-white border border-gray-200 shadow-sm rounded-lg mt-6">
                    ...
                    <p className="text-gray-600 text-lg">
                        Logged in employee hasn't uploaded any document
                    </p>
                </div>
            )}
